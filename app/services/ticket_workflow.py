...
def _require_super_admin(actor: User, action: str) -> None:
    ...
    _require_manager_level. Soft-delete stays Super-Admin-only.
    """
...
def _require_admin_level(actor: User, action: str) -> None:
    """Admin-level override actions. ADMIN, SUPER_ADMIN and the legacy OWNER
    alias all qualify; Manager and below never do."""
...
def _require_manager_level(actor: User, action: str) -> None:
    """Closing a ticket. Manager-level since 2026-09-21 — managers run the
    day-to-day queue, and leaving them unable to close it meant every stuck
    ticket waited on an Admin. Soft-delete stays Super-Admin-only."""
    if actor.role not in ADMIN_MANAGER_ROLES:
        raise HTTPException(
            status_code=403, detail=f"Only a Manager, Admin or Super Admin can {action}"
        )
...
def force_close(db: Session, ticket: Ticket, actor: User, reason: str) -> Ticket:
    """Manager/Admin/Owner override: move a ticket to CLOSED from ANY status,
    skipping signatures/PDF. A reason is required and recorded for audit."""
    _require_manager_level(actor, "close a ticket")
...
def reconcile_payment_after_charge_edit(
    ...
    balance — nothing to re-open.

    The edit can also remove the last obstacle instead: a RESOLVED ticket
    that was held only for payment, whose charges an Admin just waived to
    nothing. The close points that normally finish a ticket (last signature,
    Admin payment verification) have already run for it, so it is closed
    HERE — otherwise it would sit RESOLVED forever with nothing owed and
    nothing to verify. Returns True when this call closed the ticket; the
    caller must notify the sales rep AFTER its commit. Does not commit; the
    caller's endpoint commits once after all its changes.
    """
    if ticket.payment_status is None:
        return False  # legacy pre-tracking ticket — never gated, stays that way
    if ticket.amount_due_inr == previous_due_inr:
        return False  # edit didn't change the total — nothing to re-open
    ...
    return _close_after_charge_edit(db, ticket, actor)


def _close_after_charge_edit(db: Session, ticket: Ticket, actor: User) -> bool:
    """Close a RESOLVED ticket whose billing edit just removed the payment
    gate — signed off, nothing owed, no shipment in transit. Returns True if
    the ticket was closed. Does not commit."""
    if ticket.status != TicketStatus.RESOLVED.value:
        return False
    if payment_blocks_close(ticket):
        return False
    if not signoff_complete(ticket):
        return False
    # Same rule as signing: spare-parts shipments still in transit block the
    # RESOLVED → CLOSED transition. Here it simply waits (no error) — the
    # ticket closes via the normal delivery flow once everything lands.
    undelivered = (
    ...
    _log_event(
        ...
        from_status=prev, to_status=ticket.status,
        note="Closed after a billing edit — nothing owed and the work is already signed off.",
    )
    logger.info(
        "Ticket %s closed after charge edit by %s — nothing due, sign-off complete",
